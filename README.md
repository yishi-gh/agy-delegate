...
- 项目根自动注入，任务书直接写相对路径
- 开箱即用：工作目录自动处理（agy 无 `--cwd`，执行器用 `--add-dir` 注入）、默认自动审批写文件与执行命令（`--ask-permissions` 可收敛）
...
| `--effort` | 推理深度（如 medium / high），仅用于不带档位的 slug |
...
- Windows 没有可靠的跨进程优雅中断：超时终止只能整树硬杀（CTRL_BREAK 依赖共享控制台，无控制台环境不送达）
- 每次无头调用固定约 1.2 万 input token 开销：无法消除，多轮用 `--conversation` 续接可复用服务端缓存
